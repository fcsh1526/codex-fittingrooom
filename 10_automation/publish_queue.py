...
from canva_templates import choose_template, load_template_registry
...
def next_action_for_stage(stage):
    actions = {
        ...
        "needs_visual_revision": "Do not publish. Use the registered active Mira Canva master template, verify layer/frame compatibility, regenerate clean candidates, then test-fill before export.",
        "ready_for_canva_test": "Do not publish yet. Test-fill the registered active Mira Canva master template with selected v2 assets, review crops, then decide whether to commit.",
...
def carousel_item(run_dir, packet, asset, publish_rows, metric_rows, template=None):
    ...
    return {
        ...
        "canva_design_url": clean(packet.get("canva_design_url")),
...
def write_markdown(path, rows, runs_dir):
    ...
    if not rows:
        ...
        f"- ID: `{top['carousel_id']}`",
                ...
                f"- Canva design URL: {top.get('canva_design_url') or 'n/a'}",
                f"- Stage: `{top['stage']}`",
