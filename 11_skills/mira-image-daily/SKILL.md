...
description: Use when working on the Mira AI fashion magazine image workflow in the 人物試衣間 project: turning global Perplexity weekly fashion trend rows into daily image-generation jobs, enforcing M01/M02/M03 reference-start-image consistency, creating candidate prompts, storing generated images in the project run folder, and preparing image review sheets before Canva.
---
...
10_automation/runs/{week_id}/image_review_template.csv
```
...
3. Confirm the required reference start image exists through `mira_reference_images.csv`.
...
7. Fill the review sheet before asset selection or Canva.

Do not skip step 3. If the reference start image is missing, stop and tell the user which reference image must be created or supplied first.
...
& 'C:\Users\Brandon_ChangChien\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' `
  C:\Users\Brandon_ChangChien\.codex\skills\mira-image-daily\scripts\prepare_daily_image_job.py `
  --project-root . `
...
- Use the assigned internal model only: `M01`, `M02`, or `M03`.
- Use the model's reference start image as the identity anchor.
- Do not publish or render model IDs, names, labels, prompt notes, or reference-image metadata.
...
Reject images with supermodel distance, runway mood, luxury hotel advertising, plastic skin, obvious AI anatomy, hidden outfits, visible logos, image text, watermarks, or celebrity likeness.
...
Only mark `publishable = yes` when `outfit_clarity >= 4` and `ai_realism >= 4`.
