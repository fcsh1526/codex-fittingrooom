...
The five W29 v3 Canva duplicates were initially filled on 2026-07-16, but visual review found that generic narrow portrait sources were center-cover cropped by Canva's exact frames. W29-001 and W29-002 have now completed the corrected path: each A/B/C set was purpose-built for its assigned master, visually reviewed in Canva, approved by the user, and saved. Both are `ready_for_manual_export`; W29-003 through W29-005 remain `needs_visual_revision`. Old W29 v2 selections must not be used for new Canva work.
...
W29-001 correctly resolves to v3-B: A `1240x1350`, B `1140x560`, C `1180x1350`. W29-002 correctly resolves to v3-E: A `1120x1050`, B `1120x410`, C `1060x1010`; all three required at most 0.11% center crop. Both saved designs preserve complete hair and faces, keep the outfit focus visible, and show the full `MIRA` mark. Next production item is W29-003.
