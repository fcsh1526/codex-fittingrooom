...
The five W29 v3 Canva duplicates were filled and committed on 2026-07-16, but visual review found that narrow portrait source images are center-cover cropped by Canva's near-square frames, cutting the hairstyle/head in most designs. W29 is now `needs_visual_revision`, not ready to publish. A near-square W29-001 M01 Hero derivative passed the untouched v3-B left-frame test; its temporary one-frame Canva transaction was cancelled because the original C detail still fails. Next step: create and validate a crop-safe C derivative, then replace both affected frames together. Old W29 v2 selections must not be used for new Canva work.
